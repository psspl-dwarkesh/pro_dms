...
function LeadDialog({ saving, onClose, onSubmit }: { saving: boolean; onClose: () => void; onSubmit: (body: object) => void }) { const [form, setForm] = useState({ customerId: "", customerLabel: "", source: "web", interestedVehicle: "", expectedValue: "" }); return <WorkflowModal title="Create lead" eyebrow="Sales 360" completeLabel="Create lead" busy={saving} onClose={onClose} onComplete={() => onSubmit({ ...form, customerLabel: undefined, expectedValue: form.expectedValue ? Number(form.expectedValue) : undefined })}><div className="workflow-form-grid"><label className="workflow-form-full"><span>Customer (required)</span><CustomerPicker value={form.customerLabel} onSelect={(customer) => setForm({ ...form, customerId: customer.id, customerLabel: customer.displayName })} /></label><label><span>Source (required)</span><select value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })}><option value="web">Web enquiry</option><option value="walk-in">Walk-in</option><option value="phone">Phone</option><option value="referral">Referral</option></select></label><label><span>Vehicle or model interest</span><input value={form.interestedVehicle} onChange={(e) => setForm({ ...form, interestedVehicle: e.target.value })} /></label><label><span>Expected value (AUD)</span><input type="number" min="0" value={form.expectedValue} onChange={(e) => setForm({ ...form, expectedValue: e.target.value })} /></label></div></WorkflowModal>; }
function DriveDialog({ saving, onClose, onSubmit }: DialogProps) { const [form, setForm] = useState({ vehicleId: "", vehicleLabel: "", scheduledAt: "", status: "scheduled" }); return <WorkflowModal title="Schedule test drive" eyebrow="Sales journey" busy={saving} onClose={onClose} onComplete={() => onSubmit({ ...form, vehicleLabel: undefined })}><div className="workflow-form-grid"><label className="workflow-form-full"><span>Vehicle (required)</span><VehiclePicker value={form.vehicleLabel} onSelect={(vehicle: Vehicle) => setForm({ ...form, vehicleId: vehicle.id, vehicleLabel: `${vehicle.make} ${vehicle.model}` })} /></label><label><span>Scheduled time (required)</span><input type="datetime-local" value={form.scheduledAt} onChange={(e) => setForm({ ...form, scheduledAt: e.target.value })} /></label></div></WorkflowModal>; }
function QuoteDialog({ saving, onClose, onSubmit }: DialogProps) { const [form, setForm] = useState({ vehicleId: "", vehicleLabel: "", amount: "", status: "draft", validUntil: "" }); return <WorkflowModal title="Create quotation" eyebrow="Sales journey" completeLabel="Create quote" busy={saving} onClose={onClose} onComplete={() => onSubmit({ ...form, vehicleLabel: undefined, amount: Number(form.amount) })}><div className="workflow-form-grid"><label className="workflow-form-full"><span>Vehicle</span><VehiclePicker value={form.vehicleLabel} onSelect={(vehicle: Vehicle) => setForm({ ...form, vehicleId: vehicle.id, vehicleLabel: `${vehicle.make} ${vehicle.model}` })} /></label><label><span>Amount (AUD, required)</span><input type="number" min="0" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} /></label><label><span>Valid until</span><input type="date" value={form.validUntil} onChange={(e) => setForm({ ...form, validUntil: e.target.value })} /></label></div></WorkflowModal>; }
function FollowUpDialog({ saving, onClose, onSubmit }: DialogProps) { const [form, setForm] = useState({ channel: "call", summary: "", dueAt: "" }); return <WorkflowModal title="Add follow-up" eyebrow="Sales journey" completeLabel="Add follow-up" busy={saving} onClose={onClose} onComplete={() => onSubmit(form)}><div className="workflow-form-grid"><label><span>Channel (required)</span><select value={form.channel} onChange={(e) => setForm({ ...form, channel: e.target.value })}><option>call</option><option>email</option><option>sms</option><option>whatsapp</option><option>in-person</option></select></label><label><span>Due time (required)</span><input type="datetime-local" value={form.dueAt} onChange={(e) => setForm({ ...form, dueAt: e.target.value })} /></label><label className="workflow-form-full"><span>Next action (required)</span><textarea value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })} /></label></div></WorkflowModal>; }
function OrderDialog({ saving, onClose, onSubmit }: DialogProps) { const [form, setForm] = useState({ vehicleId: "", vehicleLabel: "", totalAmount: "" }); return <WorkflowModal title="Create sales order" eyebrow="Order and delivery" completeLabel="Create order" busy={saving} onClose={onClose} onComplete={() => onSubmit({ ...form, vehicleLabel: undefined, totalAmount: Number(form.totalAmount) })}><div className="workflow-form-grid"><label className="workflow-form-full"><span>Vehicle (required)</span><VehiclePicker value={form.vehicleLabel} onSelect={(vehicle: Vehicle) => setForm({ ...form, vehicleId: vehicle.id, vehicleLabel: `${vehicle.make} ${vehicle.model}` })} /></label><label><span>Total amount (AUD, required)</span><input type="number" min="0" value={form.totalAmount} onChange={(e) => setForm({ ...form, totalAmount: e.target.value })} /></label></div></WorkflowModal>; }
