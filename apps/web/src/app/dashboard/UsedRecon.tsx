...
function InspectionModal({ saving, onClose, onSubmit }: { saving: boolean; onClose: () => void; onSubmit: (body: object) => void }) { const [form,setForm]=useState({inspectionStatus:"passed",inspectionGrade:"good",inspectionNotes:""}); return <WorkflowModal title="Record used vehicle inspection" eyebrow="Condition gate" completeLabel="Save inspection" busy={saving} onClose={onClose} onComplete={() => onSubmit(form)}><div className="workflow-form-grid"><label><span>Outcome</span><select value={form.inspectionStatus} onChange={(e)=>setForm({...form,inspectionStatus:e.target.value})}><option value="in-progress">In progress</option><option value="passed">Passed</option><option value="failed">Failed</option></select></label><label><span>Condition grade</span><select value={form.inspectionGrade} onChange={(e)=>setForm({...form,inspectionGrade:e.target.value})}>{["excellent","good","fair","poor"].map(x=><option key={x}>{x}</option>)}</select></label><label className="full"><span>Inspection notes</span><textarea required value={form.inspectionNotes} onChange={(e)=>setForm({...form,inspectionNotes:e.target.value})} /></label></div></WorkflowModal>; }
function TaskModal({ saving, onClose, onSubmit }: { saving: boolean; onClose: () => void; onSubmit: (body: object) => void }) { const [form,setForm]=useState({category:"mechanical",description:"",supplier:"",estimatedCost:"",dueAt:""}); function submit(e?:FormEvent){e?.preventDefault(); if(form.description.trim()) onSubmit({...form,estimatedCost:Number(form.estimatedCost||0),dueAt:form.dueAt||null,status:"planned"});} return <WorkflowModal title="Add reconditioning task" eyebrow="Recon plan" completeLabel="Add task" busy={saving} onClose={onClose} onComplete={()=>submit()}><div className="workflow-form-grid"><label><span>Category</span><select value={form.category} onChange={(e)=>setForm({...form,category:e.target.value})}>{["mechanical","body","interior","tyres","detail","other"].map(x=><option key={x}>{x}</option>)}</select></label><label><span>Description</span><input required value={form.description} onChange={(e)=>setForm({...form,description:e.target.value})} /></label><label><span>Supplier</span><input value={form.supplier} onChange={(e)=>setForm({...form,supplier:e.target.value})} /></label><label><span>Estimated cost (AUD)</span><input type="number" min="0" value={form.estimatedCost} onChange={(e)=>setForm({...form,estimatedCost:e.target.value})} /></label><label><span>Due date</span><input type="datetime-local" value={form.dueAt} onChange={(e)=>setForm({...form,dueAt:e.target.value})} /></label></div></WorkflowModal>; }
function PriceModal({ saving, onClose, onSubmit }: { saving:boolean; onClose:()=>void; onSubmit:(body:object)=>void }) { const [price,setPrice]=useState(""); return <WorkflowModal title="Set retail asking price" eyebrow="Pricing" completeLabel="Update price" busy={saving} onClose={onClose} onComplete={()=>price&&onSubmit({askingPrice:Number(price),disposalChannel:"retail"})}><label><span>Asking price (AUD)</span><input autoFocus type="number" min="0" required value={price} onChange={(e)=>setPrice(e.target.value)} /></label></WorkflowModal>; }
function WholesaleModal({ saving, onClose, onSubmit }: { saving:boolean; onClose:()=>void; onSubmit:(body:object)=>void }) { const [form,setForm]=useState({wholesaleBuyer:"",wholesalePrice:""}); return <WorkflowModal title="Record wholesale disposal" eyebrow="Stock disposal" completeLabel="Mark sold wholesale" busy={saving} onClose={onClose} onComplete={()=>form.wholesaleBuyer.trim()&&form.wholesalePrice&&onSubmit({disposalChannel:"wholesale",wholesaleBuyer:form.wholesaleBuyer,wholesalePrice:Number(form.wholesalePrice)})}><div className="workflow-form-grid"><label><span>Wholesale buyer</span><input required value={form.wholesaleBuyer} onChange={(e)=>setForm({...form,wholesaleBuyer:e.target.value})} /></label><label><span>Sale price (AUD)</span><input type="number" min="0" required value={form.wholesalePrice} onChange={(e)=>setForm({...form,wholesalePrice:e.target.value})} /></label></div></WorkflowModal>; }
