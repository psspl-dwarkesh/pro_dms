...
export function CustomerView({ onNavigate }: RecordViewProps) {
  ...
  return <WorkspacePage title="Customer 360" eyebrow="Relationship intelligence" description="Search a mobile number. See the household, vehicles, value, consent and every connected interaction." action={<DataSourceBadge source={source} />}>
    <form className="record-search" onSubmit={searchCustomer}><Search size={18} /><input aria-label="Search customers" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Name, mobile, or email" /><button type="submit" disabled={loading}>Search records</button></form>
    <SearchState loading={loading} error={error} />
    <section className="customer-directory"><header><div><span>Customer directory</span><strong>{visibleCustomers.length} matching records</strong></div><div className="filter-chips">{["All","VIP","Service due","Prospect"].map((item) => <button type="button" className={segment === item ? "active" : ""} onClick={() => setSegment(item)} key={item}>{item}</button>)}</div></header><div className="customer-list-head"><span>Customer</span><span>Vehicle / interest</span><span>Lifetime value</span><span>Next action</span></div>{visibleCustomers.map((entry) => <button type="button" className={customer.displayName === entry.name ? "selected" : ""} key={entry.name} onClick={() => selectCustomer(entry)}><span className="customer-list-avatar">{entry.name.split(" ").map((part) => part[0]).join("")}</span><div><strong>{entry.name}</strong><small>{entry.mobile} · {entry.segment}</small></div><span>{entry.vehicle}</span><b>{new Intl.NumberFormat("en-AU",{style:"currency",currency:"AUD",maximumFractionDigits:0}).format(entry.value)}</b><em>{entry.next}</em><ArrowRight /></button>)}{!visibleCustomers.length && <div className="customer-list-empty"><Search />No matching directory records. Submit the search to query Neon.</div>}</section>
    {!empty && <ActionBar><button type="button" onClick={() => setModal("customer")}><UserPlus />New customer</button><button type="button" onClick={() => setModal("edit-customer")}><Edit3 />Edit profile</button><a href={`tel:${customer.mobile}`}><Phone />Call</a><a href={`https://wa.me/${customer.mobile.replace(/\D/g, "")}`} target="_blank" rel="noreferrer"><MessageCircle />WhatsApp</a><a href={`mailto:${customer.email}?subject=Your AutoAxis ownership review`}><Mail />Email</a><button type="button" onClick={() => setModal("task")}><ListChecks />Create task</button><button type="button" onClick={() => setModal("note")}><StickyNote />Add note</button><button type="button" onClick={actions.share}><Share2 />Share</button><button type="button" onClick={actions.exportRecord}><Download />Export</button><button type="button" onClick={() => setModal("portal")}><CircleUserRound />Portal</button></ActionBar>}
...
export function VehicleView({ onNavigate }: RecordViewProps) {
  ...
  return <WorkspacePage title="Vehicle 360" eyebrow="VIN lifecycle intelligence" description="One trusted history from OEM order and ownership through service, condition, valuation and resale." action={<DataSourceBadge source={source} />}>
    <form className="record-search" onSubmit={searchVehicle}><Search /><input aria-label="Search vehicles" value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="VIN, registration, make or model" /><button type="submit" disabled={loading}>Search vehicles</button></form><SearchState loading={loading} error={error} />
    <section className="vehicle-directory"><header><div><span>Vehicle directory</span><strong>{visibleVehicles.length} matching assets</strong></div><div className="filter-chips">{["All","Customer owned","In stock","Demo","Reserved"].map((item) => <button type="button" className={stockFilter === item ? "active" : ""} onClick={() => setStockFilter(item)} key={item}>{item}</button>)}</div></header><div className="vehicle-list-head"><span>Vehicle</span><span>Owner / status</span><span>Location</span><span>Value</span><span>Next action</span></div>{visibleVehicles.map((entry) => <button type="button" className={vehicle.registration === entry.registration ? "selected" : ""} key={entry.vin} onClick={() => selectVehicle(entry)}><span className="vehicle-list-icon"><CarFront /></span><div><strong>{entry.year} {entry.make} {entry.model}</strong><small>{entry.registration} · {entry.vin.slice(-8)}</small></div><span><b>{entry.owner}</b><small>{entry.status}</small></span><span>{entry.location}</span><b>{value.format(entry.value)}</b><em>{entry.next}</em><ArrowRight /></button>)}{!visibleVehicles.length && <div className="customer-list-empty"><Search />No matching assets. Change the filter or search the connected vehicle master.</div>}</section>
    {!empty && <ActionBar><button type="button" onClick={() => setModal("vehicle")}><Plus />Add vehicle</button><button type="button" onClick={() => setModal("appraisal")}><Gauge />Appraise</button><button type="button" onClick={() => setModal("auction")}><CarFront />Auction</button><button type="button" onClick={() => setModal("rental")}><CalendarDays />Rent / demo</button><button type="button" onClick={actions.share}><Share2 />Share</button><button type="button" onClick={actions.exportRecord}><Download />Export</button></ActionBar>}
