...
import { InlineError, SelectField, TextField } from "../components/forms";
...
function DialogForm({ title, children, onClose, onSave }: { title: string; children: React.ReactNode; onClose: () => void; onSave: () => Promise<void> }) {
  ...
  return <WorkflowModal title={title} eyebrow="Parts operations" completeLabel="Save" busy={busy} onClose={onClose} onComplete={submit}>{error && <InlineError>{error}</InlineError>}<div ref={fields} className="workflow-form-grid">{children}</div></WorkflowModal>;
}
const field = (label: string, props: React.InputHTMLAttributes<HTMLInputElement>) => <TextField label={label} {...props} />;
const select = (label: string, value: string, onChange: (value: string) => void, options: { value: string; label: string }[]) => <SelectField label={label} required value={value} onChange={(event) => onChange(event.target.value)}><option value="">Select…</option>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</SelectField>;
